...
def run_function(eta):
    def run(i):
        run_teleportation(i, eta, label_multiplicator=get_label_multiplicator(eta))
...
if __name__ == "__main__":
    ...
    for eta in etas:
        if not os.path.exists(result_path + "eta_%d/" % get_label_multiplicator(eta)):
            os.makedirs(result_path + "eta_%d/" % get_label_multiplicator(eta))
        p.map_async(run_function(eta), np.arange(num_agents))
    p.close()
    p.join()
